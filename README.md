...
특정 제품을 특가로 할인판매하는 이벤트 상품페이지에서 주문하기 기능을 1초당 최대 4,000명이 시도하였을 경우 서버 부하없이 응답이 처리되는가
...
1. 상품 데이터 수 : 1,700만 개
 <br/>
KOSIS(국가통계포털)의 종합몰 기준 취급 상품 수 + 21년 기준 국내 온라인 쇼핑시장 점유율 확인 후 위 근사치값으로 결정
<br>
  <br>
...
* Auto-Scaling + Load Balancer를 통한 서버 응답시간 (Response Time) 단축 


> 단계별 문제 정의
1. 상품에 대해 1초당 1,000명의 사용자가 주문을 할 경우, TPS가 120/sec대로 도출되어 기존 목표했던 130/sec 이상에 도달하지 못함.

2. 또한 쇼핑몰 성향의 서비스 특성상 상품 조회에 대한 요청은 실시간으로 계속 있을 예정인데, 해당 작업이 상품 주문 요청과 중첩되어 동작했을 때의 처리 시간 지연 가능성을 염두에 두어야 했음.

3. 위와 같은 이유로 서비스의 핵심 기능인 상품주문 요청 처리와 상품조회 요청 처리를 동시에 실행시켜 테스트 해본 결과, TPS는 이전과 유사하게 120/sec 근사치를 나타내고 있었음

⇒ DB 트래픽 처리부담으로 인해 요청처리 시간 (Processing Time)지연 ▶ TPS 하락
...
**→ 가장 확실하고 효율적인 방법이나, 오버 엔지니어링이 될 수도 있으므로, 우선 다른 차선책들을 시도해본 뒤 목표하는 측정치에 도달하지 못할 경우 해결 방법에 추가할 예정**

### 3. `요청 처리시간 (Processing Time) 단축`

- **DB Replication (부하 분산)**
    - DB Insert 기능은 Main DB로 처리
    - DB Select / Update / Delete 기능은 Replication DB로 처리
    
    **→ DB 처리작업의 부하를 분산시킴으로써, Insert 작업을 하는 ‘주문기능’과 이외의 작업을 하는 ‘조회기능’이 동시에 발생함으로써 벌어지는 요청저리 시간을 저하시키기에 직접적인 방법으로 여겨져 해결 방법에 추가**
    

- **DB Scale-up**
    - DB의 처리량이 개선된 Scale-up 버전 사용
    
    **→ 가장 확실하고 효율적인 방법이나, 오버 엔지니어링이 될 수도 있으므로, 우선 다른 차선책들을 시도해본 뒤 목표하는 측정치에 도달하지 못할 경우 해결 방법에 추가할 예정**
    

- Query 튜닝
    - Query튜닝을 통해 DB에 요청하는 Query의 처리 시간을 단축
    
    **→ 해당 작업으로 인해 포기해야하는 기술적 장점은 발생하지 않으므로 해결 방법에 추가**
    
- **Cache 이용**
    - 데이터 집계 및 계산 작업을 미리 계산하여 Caching 혹은 DB에 저장
...
![image](https://user-images.githubusercontent.com/102216495/206993236-3d5cb358-a91c-44aa-af04-44e880a4413f.png)

![image](https://user-images.githubusercontent.com/102216495/206993431-11a809f9-a701-4571-93cb-4325bbfaaaa5.png)
